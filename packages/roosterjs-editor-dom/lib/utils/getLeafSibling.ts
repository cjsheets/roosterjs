...
export function getLeafSibling(
    ...
    if (contains(rootNode, startNode)) {
        ...
        while (shouldContinue) {
            ...
            while (
                curNode &&
                (!atomTags || atomTags.indexOf(getTagOfNode(curNode)) < 0) &&
                getChild(curNode)
            ) {
 ...
 * @param atomTags (Optional) tags to be treated as atom element and won't go inside
 */
export function getNextLeafSibling(rootNode: Node, startNode: Node, atomTags?: string[]): Node {
    return getLeafSibling(rootNode, startNode, true /*isNext*/, atomTags);
}
 ...
 * @param atomTags (Optional) tags to be treated as atom element and won't go inside
 */
export function getPreviousLeafSibling(rootNode: Node, startNode: Node, atomTags?: string[]): Node {
    return getLeafSibling(rootNode, startNode, false /*isNext*/, atomTags);
}
