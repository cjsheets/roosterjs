...
export default class VListItem {
    ...
    mergeItems(items: VListItem[]) {
        ...
        const targetNodes = VListItem.wrapIfNotBlockNode(
            nodesToWrap,
            true /*checkFirst*/,
            false /*checkLast*/
        );
    ...
    writeBack(listStack: Node[]) {
        ...
        if (this.listTypes.length <= 1) {
            VListItem.wrapIfNotBlockNode(
                getTagOfNode(this.node) == 'LI'
                    ? this.getChildrenAndUnwrap(this.node)
                    : [this.node],
                true /*checkFirst*/,
                true /*checkLast*/
            );
    ...
    private static wrapIfNotBlockNode(
        nodes: Node[],
        checkFirst: boolean,
        checkLast: boolean
    ): Node[] {
    ...
    private getChildrenAndUnwrap(node: Node): Node[] {
