...
export default function createVListFromRegion(region: Region, startNode?: Node): VList {
    ...
    if (startNode) {
        let list = region.findClosestElementAncestor(startNode, ListSelector) as ListElement;
        while (list && region.contains(list.parentNode) && isListElement(list.parentNode)) {
            list = list.parentNode as ListElement;
        }
        ...
        region.getSelectedBlockElements().forEach(block => {
