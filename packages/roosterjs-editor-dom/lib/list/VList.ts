...
import isListElement from './isListElement';
...
import { Indentation, ListType, NodePosition } from 'roosterjs-editor-types';
...
export default class VList {
    ...
    constructor(private rootList: HTMLOListElement | HTMLUListElement) {
        ...
        // </ol>
        // ```
        VList.moveChildNodesToLi(this.rootList);
        queryElements(this.rootList, 'ol,ul', VList.moveChildNodesToLi);
        ...
        // </ol>
        // ```
        queryElements(this.rootList, 'li', VList.moveLiToList);

        this.iterateItems(this.rootList);
    }

    /**
     * Create a new instance of VList from node array. Each node will be treated as a potential list item,
     * and will be wrapped with LI tag if it hasn't been wrapped.
     * @param nodes The nodes to create VList from. For null or empty array, this will return null
     */
    static createFromNodes(nodes: Node[]): VList {
        ...
        const listNode = firstNode.ownerDocument.createElement('ol'); // Either OL or UL is ok here
        firstNode.parentNode?.insertBefore(listNode, firstNode);

        // Create the VList and append items
        const vList = new VList(listNode);
        nodes.forEach((node) => vList.appendItem(node, ListType.None));
    ...
    writeBack() {
        ...
        let listStack: Node[] = [doc.createDocumentFragment()];
    ...
    private iterateItems(
        ...
        for (let item = list.firstChild; !!item; item = item.nextSibling) {
            ...
            if (isListElement(item)) {
                this.iterateItems(item as HTMLOListElement | HTMLUListElement, newListTypes);
            } else {
    ...
    private static moveChildNodesToLi(list: HTMLOListElement | HTMLUListElement) {
    ...
    private static moveLiToList(li: HTMLLIElement) {
